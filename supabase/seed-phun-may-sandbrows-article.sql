...
set
  detail_json = jsonb_set(
    ...
    '{title}',
    to_jsonb('Phun Mày SANDBROWS')
  ),
  detail_json = jsonb_set(
    coalesce(detail_json, '{}'::jsonb),
    '{intro}',
    to_jsonb('SANDBROWS là kỹ thuật phun mày rải hạt (ombre/powder) cho hiệu ứng phủ phấn mịn, đều màu và tự nhiên — gọn gàng mà không bị cứng hay bệt khối.')
  ),
  detail_json = jsonb_set(
    coalesce(detail_json, '{}'::jsonb),
    '{metaDescription}',
    to_jsonb('Phun Mày SANDBROWS: kỹ thuật phun mày rải hạt cho hiệu ứng phủ phấn mịn, đều màu, tự nhiên. Thiết kế dáng theo khuôn mặt, lên màu ổn định. Tư vấn & đặt lịch tại Phuoc Lai Luxury (Vũng Tàu).')
  ),
  detail_json = jsonb_set(
    coalesce(detail_json, '{}'::jsonb),
    '{seo}',
