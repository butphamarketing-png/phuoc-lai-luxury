...
import { getPublicTrainingPath } from "@/data/catalog";
...
export default function TrainingDetail() {
  ...
  const { data: related = [] } = usePublicTraining();
  ...
  const sidebarItems = related.filter((c) => c.slug !== slug).slice(0, 4);
        ...
        <div className="flex flex-col lg:flex-row gap-20">
          <div className="lg:w-2/3">
            ...
            >
              ...
              <span className="flex items-center gap-3">
                <User size={14} className="text-black/20" /> {data.instructor}
              </span>
              <span className="flex items-center gap-3">
                <Clock size={14} className="text-black/20" /> {data.duration}
              </span>
            </motion.div>
            ...
            <div className="sticky top-32 space-y-12">
              <div className="rounded-3xl bg-[#FAFAFA] p-10 border border-black/[0.03] luxury-shadow">
                ...
                <div className="space-y-8">
                  {sidebarItems.map((item) => (
                    <Link
                      key={item.slug}
                      href={getPublicTrainingPath(item.slug)}
              ...
              <div className="rounded-3xl bg-[#1A1A1A] p-10 text-white shadow-2xl overflow-hidden relative group">
                ...
                <Award
                  size={40}
                  strokeWidth={1}
                  className="mb-8 opacity-40 relative z-10"
                />
                ...
                <p className="text-[11px] font-light text-white/50 leading-relaxed mb-10 uppercase tracking-[0.2em] relative z-10">
                  Hoàn thành khóa học và nhận bằng Master từ Phuoc Lai Luxury
                  Academy.
                </p>
                <Button
                  asChild
